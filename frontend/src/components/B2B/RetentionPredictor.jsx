          ...
          <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-baseline gap-1">
                  <span className={`text-xl font-extrabold ${tiempoTraslado > 45 ? 'text-rose-600' : 'text-sky-600'}`}>
                    {tiempoTraslado}
                  </span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
              ...
              >
                <div className={`mt-0.5 p-1.5 rounded-lg ${turnosFijos ? 'bg-emerald-600 text-white' : 'bg-slate-200 text-slate-500'}`}>
                  <CalendarCheck className="w-4 h-4" />
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-slate-900">Turnos Fijos</span>
                    <span className="text-[10px] px-1.5 py-0.2 rounded bg-emerald-100 text-emerald-800 font-bold">+2.0m</span>
                  </div>
              ...
              >
                <div className={`mt-0.5 p-1.5 rounded-lg ${apoyoInea ? 'bg-purple-600 text-white' : 'bg-slate-200 text-slate-500'}`}>
                  <GraduationCap className="w-4 h-4" />
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-slate-900">Apoyo INEA</span>
                    <span className="text-[10px] px-1.5 py-0.2 rounded bg-purple-100 text-purple-800 font-bold">+3.5m</span>
                  </div>
                  <p className="text-[11px] text-slate-500 mt-1 leading-snug">
            ...
            <div className="pt-2 flex justify-end">
              <button
                ...
                className="text-xs text-slate-400 hover:text-slate-700 flex items-center gap-1 transition font-medium"
              >
            ...
            {prediction && (
              <div className="space-y-2.5 bg-slate-50 p-4 rounded-2xl border border-slate-200/80">
                ...
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-600">Permanencia Base del Mercado</span>
                  <span className="font-bold text-slate-900">+{prediction.base_months.toFixed(1)} meses</span>
                </div>

                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-600">Impacto Salarial (${sueldo}/sem)</span>
                  <span className={`font-bold ${prediction.sueldo_impact >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                    {prediction.sueldo_impact >= 0 ? `+${prediction.sueldo_impact.toFixed(2)}` : prediction.sueldo_impact.toFixed(2)} meses
                  </span>
                </div>

                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-600">Distancia ({tiempoTraslado} min)</span>
                  <span className={`font-bold ${prediction.traslado_impact < 0 ? 'text-rose-600' : 'text-slate-600'}`}>
                    {prediction.traslado_impact.toFixed(2)} meses
                  </span>
                </div>

                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-600">Turnos Fijos</span>
                  <span className={`font-bold ${turnosFijos ? 'text-emerald-600' : 'text-slate-400'}`}>
                    {turnosFijos ? `+${prediction.turnos_impact.toFixed(1)} meses` : '0.0 meses'}
                  </span>
                </div>

                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-600">Programa Aula / Horarios INEA</span>
                  <span className={`font-bold ${apoyoInea ? 'text-purple-600' : 'text-slate-400'}`}>
